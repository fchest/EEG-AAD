...
def load_model(path, subject_id):
    model_save_path = os.path.join(path, f"S{subject_id}.pt")
    if not os.path.exists(model_save_path):
        raise FileNotFoundError(f" Model weight not found: {model_save_path}")
    print(f" Loaded model for subject S{subject_id}: {model_save_path}")
    # model = torch.load(model_save_path, map_location=device)
    model = torch.load(model_save_path)
    return model

def run_inference(args, testdata, subject_id):
    ...
    model.to(device)
    model = load_model(args.resume, subject_id)
    model.eval()

    BaselineNet = Trynetwork(model, batch_size=args.batch_size)
    pre_labels = BaselineNet.inference(test_loader)
...
def parse_args():
    ...
    p.add_argument("--model_path", 
                   type=str, 
                   default="savamodel/cross-session/DARNet", #Your model_path 
                   help="Checkpoint path (.pt/.pth)")
    p.add_argument( "--data_path", 
                   type=str, 
                   default="/testdata/",  #Your data_path
                   help="EEG source (# eeg-aad-challenge2025-task2-baselines-master/dataset/test_data )")
    p.add_argument("--out_csv", 
                   type=str, 
                   default="./results_task2/cross_session", 
...
if __name__ == "__main__":
    ...
    for id in sub_ids:    
        ...
        pre_lables = run_inference(args, testdata, id)
