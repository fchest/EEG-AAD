...
def initiate(args, train_loader, valid_loader, test_loader, subject):
    model = DARNet(config)    #change your model here
...
def train_model(settings, args, train_loader, valid_loader, test_loader2, subject):
    ...
    #     df = pd.DataFrame({
    #     'ID': [f"ID:{i}" for i in range(len(all_preds))],  
    #     'Predicted': [f"Predicted:{p}" for p in all_preds] 
    # })
    #     df.to_csv(f'results_Task2_{args.name}.csv', index=False)
    ...
    for epoch in tqdm(range(1, args.max_epoch + 1), desc='Training Epoch', leave=False):
        ...
        print()
        print(
            'Epoch {:2d} Finsh | Subject {} | Train Loss {:5.4f} | Train Acc {:5.4f} '.format(
                epoch,
                args.name,
                train_loss,
                train_acc
    ...
    test_loss, test_acc = evaluate(model, criterion, test=False)   #test=True prediction
...
def main(name="s1", time_len=1, dataset="MM-AAD"):
    setup_seed(42)
    args = DotMap()
    args.name = name
    args.max_epoch = 100
...
if __name__ == "__main__":
    multiple = 1
    process = []
    code_path = '/media/gaoyoudian/DXG/Task2_baseline/'#your filepath
    dataset = 'MM-AAD'
    config['time_len'] = 1
    names = ['S' + str(i+1) for i in range(30)]
    accs = []
    for name in names:
        time_len = config['time_len']
        print(name)
        loss, acc = main(name=name, time_len = time_len, dataset = dataset )
        accs.append(acc)
    avg_acc = sum(accs) / len(accs)
    df = pd.DataFrame({'subject': names + ['average'], 'acc': accs + [avg_acc]})
    csv_path = code_path + 'accs_' + str(time_len) + 's.csv'
